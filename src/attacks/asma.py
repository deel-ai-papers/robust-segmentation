...
def asma(
    ...
    for i in range(num_steps):
        ...
        logit_loss = (
            logits.gather(1, labels_.unsqueeze(1))
            .squeeze(1)
            .mul(masks & (pred != labels_))
            .sum()
        )
        ...
        if callback:
            callback.accumulate_line(
                "logit_loss", i, logit_loss.mean(), title=attack_name + " - Logit loss"
            )
            callback.accumulate_line(
                ["adv%", "best_adv%"],
                i,
            ...
            callback.accumulate_line(
                ["ℓ2", "best ℓ2"],
                i,
                [l2_squared.detach().sqrt().mean(), best_dist.sqrt().mean()],
                title=attack_name + " - L2 Norms",
            )
            callback.accumulate_line(
                "lr", i, pert_mul, title=attack_name + " - Step size"
            )
