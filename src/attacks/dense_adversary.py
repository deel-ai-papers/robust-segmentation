    ...
    for i in range(max_iter):
        ...
        if i == 0:
            ...
            labels_infhot = torch.zeros_like(logits.detach()).scatter(
                1, masked_labels.unsqueeze(1), float("inf")
            )

        dl = multiplier * difference_of_logits(
            logits,
            labels=masked_labels[active_inputs],
            labels_infhot=labels_infhot[active_inputs],
        )
        pixel_is_adv = dl < 0

        active_masks = masks[active_inputs]
        adv_percent = (pixel_is_adv & active_masks).flatten(1).sum(dim=1) / masks_sum[
            active_inputs
        ]
        is_adv = adv_percent >= adv_threshold
        adv_found[active_inputs] = is_adv
        best_adv[active_inputs] = torch.where(
            batch_view(is_adv), adv_inputs_.detach(), best_adv[active_inputs]
        )

        if callback:
            callback.accumulate_line(
                "dl", i, dl[active_masks].mean(), title=f"DAG (p={p}, γ={γ}) - DL"
            )
            callback.accumulate_line(
                f"L{p}",
                i,
                r.flatten(1).norm(p=p, dim=1).mean(),
                title=f"DAG (p={p}, γ={γ}) - Norm",
            )
            callback.accumulate_line(
                "adv%", i, adv_percent.mean(), title=f"DAG (p={p}, γ={γ}) - Adv percent"
            )
