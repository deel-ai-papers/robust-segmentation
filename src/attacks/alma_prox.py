...
import torch
from adv_lib.attacks.augmented_lagrangian import (
    _distances,
    all_penalties,
    difference_of_logits_ratio,
    init_lr_finder,
)
...
def alma(
    ...
    for i in range(num_steps):
        ...
        if constraint_masking:
            ...
            if k.any():
                top_constraints = (
                    dlr.detach().sub(masks_inf).flatten(1).topk(k=k.max()).values
                )
        ...
        if i:
            new_μ = grad(
                penalty(dlr, ρ, μ)[constraint_mask].sum(), dlr, only_inputs=True
            )[0]
        ...
        if init_lr_distance is not None and i == 0:
            ...
            lr = init_lr_finder(
                inputs, δ_grad, dist_func, target_distance=init_lr_distance
            )

        exp_decay = lr_reduction ** (
            (i - step_found).clamp_(min=0) / (num_steps - step_found)
        )
