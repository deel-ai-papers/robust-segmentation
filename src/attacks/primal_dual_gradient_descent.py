...
from adv_lib.attacks.primal_dual_gradient_descent import (
    l0_proximal_,
    l1_proximal,
    l23_proximal,
    l2_proximal_,
...
from adv_lib.distances.lp_norms import (
    l0_distances,
    l1_distances,
    l2_distances,
    linf_distances,
...
def pdgd(
    ...
    for i in range(num_steps):
        ...
        if i == 0:
            ...
            masks_inf = torch.zeros_like(masks, dtype=torch.float).masked_fill_(
                ~masks, float("inf")
            )
            labels_infhot = torch.zeros_like(logits.detach()).scatter(
                1, labels_.unsqueeze(1), float("inf")
            )
            dl_func = partial(
                difference_of_logits, labels=labels_, labels_infhot=labels_infhot
            )
            ...
            k = (
                (1 - adv_threshold) * masks_sum
            ).long()  # number of constraints that can be violated
        ...
        if constraint_masking:
            ...
            if k.any():
                top_constraints = (
                    m_y.detach().sub(masks_inf).flatten(1).topk(k=k.max()).values
                )
                ξ = top_constraints.gather(1, k.unsqueeze(1) - 1).squeeze(1)
                constraint_mask = masks & (m_y <= ξ.view(-1, 1, 1))
                constraint_inf_mask.fill_(0).masked_fill_(
                    ~constraint_mask, float("inf")
                )
        ...
        θ_λ = dual_lr * (
            (num_steps - 1 - i) / (num_steps - 1) * (1 - dual_lr_decrease)
            + dual_lr_decrease
        )
        λ.add_(grad_λ, alpha=θ_λ).clamp_(
            min=log_min_dual_ratio, max=-log_min_dual_ratio
        )
        λ[~masks] = -float("inf")

        if callback is not None:
            callback.accumulate_line(
                "m_y", i, m_y.mean(), title=f"{attack_name} - Logit difference"
            )
            callback.accumulate_line(
                "1 - sum(λ)", i, λ_1.mean(), title=f"{attack_name} - Dual variables"
            )
            callback.accumulate_line(
                ["θ_r", "θ_λ"],
                i,
            ...
            callback.accumulate_line(
                ["l2", "best_l2"],
                i,
                [l2.mean(), best_l2.mean()],
                title=f"{attack_name} - L2 norms",
            )
            callback.accumulate_line(
                ["adv%", "best_adv%"],
                i,
                [adv_percent.mean(), best_adv_percent.mean()],
...
def pdpgd(
    ...
    proximity_operator = _proximal_operator[
        norm if proximal_operator is None else proximal_operator
    ]
    log_min_dual_ratio = math.log(dual_min_ratio)
    ...
    for i in range(num_steps):
        ...
        for _ in range(proximal_steps):
            ...
            z_new = proximity_operator(
                z_curr.addcmul(H_div, z_curr - r.detach(), value=-1), batch_view(μ)
            )
            z_new.add_(inputs).clamp_(min=0, max=1).sub_(inputs)

            z_curr = torch.where(batch_view(ε > ε_threshold), z_new, z_prev)
            ε = torch.norm((z_curr - z_prev).flatten(1), p=2, dim=1, out=ε).div_(
                z_curr.flatten(1).norm(p=2, dim=1)
            )
        ...
        if callback is not None:
            ...
            callback.accumulate_line(
                [f"l{norm}", f"best_l{norm}"],
                i,
                [dist.mean(), best_dist.mean()],
