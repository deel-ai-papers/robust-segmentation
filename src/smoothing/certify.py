"""
Taken from https://github.com/eth-sri/segmentation-smoothing/blob/main/code/multiclassify_utils.py
"""
...
def map_test(args):
    sample, n0, n, num_classes, tau = (
        args  # arguments a tuple to be compatible with multiprocessing map
    )
    c, cnt = mode(sample, n0, num_classes)
    p = sps.binomtest(cnt, n, tau, alternative="greater").pvalue
    return c, p


def map_testNaiveMulti(args):
    sample, n0, n, num_classes, alpha = (
        args  # arguments a tuple to be compatible with multiprocessing map
    )
    c, cnt = mode(sample, n0, num_classes)
    pc = statsmodels.stats.proportion.proportion_confint(
        cnt, n, alpha=2 * alpha, method="beta"
    )[0]
    return c, pc
...
def certify(
    ...
    if parallel:
        with mp.Pool() as pool:
            c_ps = pool.map(
                map_test, [(sample[:, i], n0, n, num_classes, tau) for i in range(K)]
            )
    else:
        c_ps = map(
            map_test, [(sample[:, i], n0, n, num_classes, tau) for i in range(K)]
...
def certify_naiveMulti(
    num_classes, sample, n0, n, sigma, tau, alpha, abstain=-1, parallel=False
):
    assert isinstance(sample, np.ndarray)
    shape = sample.shape
    assert len(shape) == 2
    ...
    if parallel:
        with mp.Pool() as pool:
            c_ps = pool.map(
                map_testNaiveMulti,
                [(sample[:, i], n0, n, num_classes, alpha) for i in range(K)],
            )
    else:
        c_ps = map(
            map_testNaiveMulti,
...
def certify_naiveCarthesian(sample, n0, n, sigma, alpha, abstain=-1, parallel=False):
    ...
    pc = statsmodels.stats.proportion.proportion_confint(
        nc, n, alpha=2 * alpha, method="beta"
    )[0]
    if pc >= 0.5:
...
class Log:
    def __init__(self, name):
        ...
        self.folder_path = self.log_dir / (
            name + "_" + now.strftime("%Y_%m_%d_%H:%M:%S")
        )
...
def setup_segmentation_args(parser):
    ...
    parser.add_argument(
        "--sigma", type=float, default=0.25, help="sigma for randomized smoothing"
    )
    parser.add_argument(
        "--tau",
        type=float,
        default=[0.75],
        nargs="+",
        help="tau for randomized smoothing inference",
    )
    parser.add_argument(
        "-n",
        type=int,
    ...
    parser.add_argument(
        "--baseline", type=str2bool, default=False, help="just run baseline"
    )
