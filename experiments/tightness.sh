...
NUM_STEPS_ATTACK=1000
NUM_STEPS_SMOOTH_ATTACK=1
NUM_SAMPLES_SMOOTH_ATTACK=1
ALPHA=0.001
...
for EPSILON in 0.1 0.5 1.0 1.5; do
    ...
    for TAU in 3.0; do
        CUDA_VISIBLE_DEVICES=1 python scripts/attack.py --dataset iiit_pets \
            ...
            --output_csv data/attack_tightness.csv
    done

    # 2. Smooth Attack - Matching sigmas mapped per epsilon
    case $EPSILON in
        0.1) SIGMAS="0.2 0.5 0.8" ;;
        0.5) SIGMAS="0.2 0.5 0.8" ;;
        1.0) SIGMAS="0.2 0.5 0.8" ;;
        1.5) SIGMAS="0.2 0.5 0.8" ;;
    ...
    for SIGMA in $SIGMAS; do
        CUDA_VISIBLE_DEVICES=0 python scripts/smooth_attack.py --dataset iiit_pets \
            ...
            --sigma $SIGMA --n0 50 --n 250 --tau 0.0 --alpha $ALPHA \
            --epsilon $EPSILON --num_steps $NUM_STEPS_SMOOTH_ATTACK --n_attack_samples $NUM_SAMPLES_SMOOTH_ATTACK \
            --num_batches $NUM_BATCHES_SMOOTH --output_csv data/fig_smooth_tightness.csv
    done
